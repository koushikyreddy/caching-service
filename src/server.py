...
class ApiException(Exception):
    ...
    def __init__(
        self, message, status_code=None, payload=None
    ):
...
def get(key):
    ...
    if val is not None:
        ...
        raise ApiException(
            message="Could not find key '{}' in cache".format(
                key
            ),
            status_code=404,
        )
...
def set(key):
    try:
        ...
        if 'value' in payload:
            ...
            raise ApiException(
                "'value' key not found in JSON payload"
            )
...
if __name__ == '__main__':
    app.run(
        host='0.0.0.0', port=os.getenv("SERVER_PORT", 5000)
    )
